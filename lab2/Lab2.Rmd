...
\pagebreak
Note that there are some data points that seem to not make sense, for example a 3-person, 0-vehicle household makes more HBO and HBSHOP trips than a 4-person, 0-vehicle household. We had to adjust some of these trip rates to account for that intuition. Our new tables are presented below:
...
```{=latex NHTS_table}
\begin{tabular}[c c]
\textbf{Purpose} & \textbf{Weighted Survey Trips}\\
HBW & \(118,653\)\\
HBO & \(267,987\)\\
HBSHOP & \(129,614\)
\end{tabular}
...
```{r hhprod1}
read.dbf("dbf/HH_PROD1.dbf") %>% head()
```

Adding up each trip by purpose gives us:
```{r sum1}
read.dbf("dbf/HH_PROD1.dbf")[,-1] %>% colSums()
```

These are obviously not very close to the targets, so we need to do some adjusting. We began by adjusting the rates with the largest confidence intervals first, and these are the results we got:

```{r hhprod2}
read.dbf("dbf/HH_PROD2.dbf") %>% head()
read.dbf("dbf/HH_PROD2.dbf")[,-1] %>% colSums()
```